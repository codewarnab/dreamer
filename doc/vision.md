...
5. **Writes** human-actionable proposals to a per-project `todos.md`. The user ratifies; `dreamer` never edits code itself.
...
- **Provider-agnostic.** The analysis LLM is reached either via [ACP](https://agentclientprotocol.com) (one adapter, many agents) or via per-vendor adapters where native SDK/CLI is meaningfully faster. Copilot SDK, Claude CLI, Gemini SDK/CLI, Kiro (ACP), Cursor CLI all sit behind the same `Provider` interface. No third-party scraping; no account-ban risk.
...
- `dreamer daemon` — periodic scan of all configured projects (existing surface; carried forward).
- `dreamer ls-chats`, `dreamer config init` — unchanged.
...
- Auto-applying guardrails (writing eslintrc/golangci config, generating test files).
...
- Live tool calls during analysis — context is pre-baked into the prompt; the LLM does not make mid-analysis tool calls. (Codebase reads happen via the provider's read sandbox only when the orchestrator's grounding step asks for a specific symbol.)
